...
public class SwaggerConverterTest {
    ...
    public void canConvertXsd() throws IOException {
        ...
        final URL url = Resources.getResource("VacuumdConstants.VACUUMD_XSD");
        try (InputStream is = url.openStream()) {
            schemaCol.read(new StreamSource(is));
        }

        XsdModelConverter xsdModelConverter = new XsdModelConverter();
        ConfigItem configItem = xsdModelConverter.convert(schemaCol, "VacuumdConstants.VACUUMD_ELEMENT");

        SwaggerConverter swaggerConverter = new SwaggerConverter();
        String openapiStr = swaggerConverter.convertToString(configItem, "/VacuumdConfiguration");

        final String expectedSwaggerJson = Resources.toString(
                Resources.getResource("swagger.generated.json"), StandardCharsets.UTF_8);

        System.out.println("Actual JSON: " + openapiStr);
